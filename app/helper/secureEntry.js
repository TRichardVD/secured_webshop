 ...
 * @param {number} minLength - Longueur minimale autorisée pour l'entrée.
 ...
 */
const secureInputValidation = function (input, minLength = 8, maxLength = 255) {
    return new Promise((resolve, reject) => {
        ...
        if (threats.length > 0) {
            reject(
                `Menace détectée : ${threats.map((t) => `"${t}"`).join(', ')}`
            );
            return;
        }
        ...
        if (inputStr.length < minLength) {
            reject(
                ...
                } caractère.s (min ${minLength} caractères)`
            );
            return;
        }
