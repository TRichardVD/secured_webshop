    ...
    try {
        ...
        return res.redirect(
            `/login?message=${encodeURIComponent(
                'Username or Password invalide : ' + err
            )}`
        );
    }
...
router.post('/api/deconnection', async (req, res) => {
    ...
    try {
        ...
        return res.redirect(
            '/login?message=Vous avez été déconnecté avec succès'
        );
    } catch (err) {
    ...
    async (req, res) => {
        ...
        if (name) {
            try {
                ...
                secureName = await secureEntry.secureInputValidation(
                    name,
                    0,
                    100
                );
